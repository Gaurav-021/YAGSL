...
public class Vision {
    ...
    public Vision(){
        ...
        try {
            aprilTagFieldLayout = AprilTagFieldLayout.loadFromResource(AprilTagFields.k2023ChargedUp.m_resourceFile); // is this right? The docs show "AprilTagFields.k2023ChargedUp.m_resourceFile"
          } catch (IOException e) {
            e.printStackTrace();
        }
        poseEstimator = new PhotonPoseEstimator(aprilTagFieldLayout, PoseStrategy.MULTI_TAG_PNP_ON_COPROCESSOR, aprilTagCameraBack, robotToCam);
    }    
    ...
    /**
     * Gets the Estimated Robot Pose from the apriltags in Pose2D.
    ...
    public Pose2d getEstimatedRobotPose() {
        if (estimatedPose == null || estimatedPose.isEmpty()) {
            ...
            return new Pose2d(estimatedPose.get().estimatedPose.getX(), estimatedPose.get().estimatedPose.getY(), new Rotation2d(estimatedPose.get().estimatedPose.getRotation().getX(), estimatedPose.get().estimatedPose.getRotation().getY()));        
